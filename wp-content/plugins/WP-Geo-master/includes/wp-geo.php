...
class WPGeo {
	...
	function WPGeo() {
		...
		if ( 'leaflet' == $this->get_api_string() ) { /* #riqua */
			include_once( WPGEO_DIR . 'api/leaflet/leaflet.php' );
			$this->api = new WPGeo_API_Leaflet();		
			}elseif ( 'googlemapsv3' == $this->get_api_string() ) {
	...
	function includeGoogleMapsJavaScriptAPI() {
		...
		if ( 'leaflet' == $this->get_api_string() ) { // #riqua
			wp_register_style( 'leaflet_css', WPGEO_URL . 'css/leaflet.css', false, '0.6.4' );
			wp_register_script( 'leaflet', WPGEO_URL . 'js/leaflet.js', false, '0.6.4' );
			wp_register_script( 'wpgeo', WPGEO_URL . 'js/wp-geo.leaflet.js', array( 'jquery', 'wpgeo_tooltip' ), $this->version );
			wp_register_script( 'wpgeo_admin_post_googlemaps3', WPGEO_URL . 'api/leaflet/js/admin-post.js', array( 'jquery', 'wpgeo_admin_post', 'leaflet' ), $this->version );
		} elseif ( 'googlemapsv3' == $this->get_api_string() ) {
		...
		if (  ( $wpgeo->show_maps() || $wpgeo->widget_is_active() ) && ( $wpgeo->checkGoogleAPIKey() || $wpgeo->get_api_string() == 'leaflet' ) ) {
