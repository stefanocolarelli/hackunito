	...
	function renderMapJS( $map_id = false ) {
		...
		if ( count( $this->points ) > 0 ) {
			for ( $i = 0; $i < count( $this->points ); $i++ ) {
				...
				// @todo Tooltip, icon and link for leaflet+OSM
				$js_markers_leaflet .= 'var marker_' . $map_id .'_' . $i . ' = new L.Marker( new L.LatLng(' . $this->points[$i]->coord->get_delimited() . '));' . "\n";
				$js_markers_leaflet .= 'var marker_' . $map_id .'_' . $i . '.addTo(' . $map_id . ');' . "\n";
				//$js_markers_leaflet .= 'bounds.extend(new google.maps.LatLng(' . $this->points[$i]->coord->get_delimited() . '));' . "\n";
		...
		// Map Javascript
		if ( 'leaflet' == $wpgeo->get_api_string() ) {
			$tileOSM = 'http://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png';
			$js ='if (document.getElementById("' . $div . '")) {
					map_' . $map_id . ' = new L.Map(document.getElementById("' . $div . '");
					// create the tile layer with correct attribution
					var osmUrl=\''.$tileOSM.'\';
					var osm = new L.TileLayer(osmUrl, {minZoom: 8, maxZoom: 12});		
					// start the map in South-East England
					map_' . $map_id . '.setView(new L.LatLng(51.3, 0.7),9);
					' . apply_filters( 'wpgeo_map_js_preoverlays', '', 'map_' . $map_id ) . '
					' . $js_markers_leaflet . '
					' . $js_polyline_leaflet . '
					' . $js_center_leaflet . '
					map_' . $map_id . '.addLayer(osm);
			}';
		}elseif ( 'googlemapsv3' == $wpgeo->get_api_string() ) {
				$js = '
